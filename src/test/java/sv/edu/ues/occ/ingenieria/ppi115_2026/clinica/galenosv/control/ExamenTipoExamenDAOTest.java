...
class ExamenTipoExamenDAOTest {
    ...
    @Mock
    private CriteriaQuery<ExamenTipoExamen> cq;

    @Mock
    private CriteriaQuery<Long> cqLong;

    @Mock
    private Root<ExamenTipoExamen> root;
    ...
    @BeforeEach
    void setUp() {
        dao = new ExamenTipoExamenDAO(em);
    }

    @Test
    void testConstructorsAndGetEntityManager() {
        ExamenTipoExamenDAO defaultDao = new ExamenTipoExamenDAO();
    ...
    @Test
    void testCreate() {
        ExamenTipoExamen entity = new ExamenTipoExamen(UUID.randomUUID());
    ...
    void testFindById() {
        ...
        ExamenTipoExamen expected = new ExamenTipoExamen(id);
        when(em.find(ExamenTipoExamen.class, id)).thenReturn(expected);

        ExamenTipoExamen actual = dao.findById(id);

        assertNotNull(actual);
        assertEquals(id, actual.getIdExamenTipoExamen());
        verify(em, times(1)).find(ExamenTipoExamen.class, id);
    }
    ...
    void testFindRange() {
        ...
        List<ExamenTipoExamen> result = dao.findRange(0, 10);
    ...
    void testCount() {
        ...
        when(cqLong.from(ExamenTipoExamen.class)).thenReturn(root);
